			...
			success: function(result) {
			    ...
			    success: function(result) {
					...
					for (var i = stopRes.length - 1; i >= 0; i--) {
						if(stopRes[i].status === 'complete' || stopRes[i].status === "error") {
							$("div[id*=" + stopRes[i].gid + "]").remove();
		            		stopRes.splice(i,1);
		            	}
		            }
		            if (activeRes.length + waitRes.length + stopRes.length === 0) {
		            	$("#activetpl").html('No ongoing, error or paused task');
...
function printActive(r) {
	...
	for (var i = 0; i < r.length; i++) {
		...
		if ($("#" + tplbarID).html()==null){
			...
			var progbar = progressJs("#" + tplbarID);
			progbar.setOptions({overlayMode: true, theme: 'blueOverlayRadiusWithPercentBar'});
			progbar.start();
			$("#" + tplbarID).after("<br /><br />");
		}
		if (isNaN(r[i].completedLength) || r[i].completedLength == 0) {
			progressJs("#" + tplbarID).set(0);
		} else { 
...
function printQueue(r) {
	...
	for (var i = 0; i < r.length; i++) {
		...
		if ($("#" + tplbarID).html()==null){
			...
			$("#" + tplbarID).css("width","20%");
			var progbar = progressJs("#" + tplbarID);
			progbar.setOptions({overlayMode: true, theme: 'blueOverlayRadiusWithPercentBar'}).start();
			$("#" + tplbarID).after("<br /><br />");
		}
		if (isNaN(r[i].completedLength) || r[i].completedLength == 0){
			progressJs("#" + tplbarID).set(0);
...
function printError(r) {
	...
	for (var i = 0; i < r.length; i++) {
		...
		if ($("#" + tplbarID).html()==null){
			...
			$("#" + tplbarID).css("width","20%");
			var progbar = progressJs("#" + tplbarID);
			progbar.setOptions({overlayMode: true, theme: 'redOverlayRadiusWithPercentBar'}).start();
			$("#" + tplbarID).after("<br /><br />");
		}
		if (isNaN(r[i].completedLength) || r[i].completedLength == 0){
			progressJs("#" + tplbarID).set(0);
...
writeUI();
var uirefrsh = setInterval(writeUI, 1500);